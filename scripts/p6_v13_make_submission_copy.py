# -*- coding: utf-8 -*-
'''
从 V13 留档版生成"提交版"（同 V12 的分版规则，可复跑）。
规则：
...
DST = 'results/P6_FJNSF_APPLICATION_SUBMISSION_V13_提交版.md'

lines = io.open(SRC, encoding='utf-8').read().split('\n')

# ① 首个 '---' 之前只留标题
sep = lines.index('---')
title = lines[0].replace(' · 修订版 V13', '')
...
assert '未被 JCR 收录、无 JCR 影响因子、亦未进入中科院 SCI 期刊分区表' in out, '本轮期刊层级改写未进入提交版'
