# -*- coding: utf-8 -*-
'''
从 V15 留档版生成"提交版"（同 V12 的分版规则，可复跑）。
规则：
  ① 保留标题，删除标题与首个 '---' 之间的全部 '>' 元信息块；
  ② 清扫指向被删章节（§13）的引用（按需替换，不强制命中 1 次），并在生成后断言 §13 字样零残留；
  ③ 删除 §13 整节；
  ④ 删除文末作者脚注；
  ⑤ 在标题下插入一行最小"编者说明"。
V15 新增断言：A2 框架（不以 ADRA2A 单基因大幅上调为前提）、期刊层级改写、中文期刊层级披露均进入提交版。
'''
import io
import re
import sys

SRC = 'results/P6_FJNSF_APPLICATION_SUBMISSION_V15.md'
DST = 'results/P6_FJNSF_APPLICATION_SUBMISSION_V15_提交版.md'

lines = io.open(SRC, encoding='utf-8').read().split('\n')

# ① 首个 '---' 之前只留标题
sep = lines.index('---')
title = lines[0].replace(' · 修订版 V15', '')
...
assert '不以 ADRA2A 单基因大幅上调为成立前提' in out, 'V15 A2 框架（假说不以单基因上调为成立前提）未进入提交版'
assert 'ADRA2A 在 SMIR 主模型中单基因变化幅度小且不显著（lfc = +0.077，FDR = 0.934）' in out, 'V15 A1 模型依据诚实注记未进入提交版'

print('提交版已生成：%s' % DST)
print('行数：%d；§13 残留：0；含编者说明：%s；参考文献 %d 条连续；V15 A2/A1 断言通过'
