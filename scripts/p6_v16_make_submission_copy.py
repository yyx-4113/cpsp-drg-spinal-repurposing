# -*- coding: utf-8 -*-
'''
从 V16 留档版生成"提交版"（同 V12 的分版规则，可复跑）。
规则：
  ① 保留标题，删除标题与首个 '---' 之间的全部 '>' 元信息块；
  ② 清扫指向被删章节（§13）的引用（按需替换），并在生成后断言 §13 字样零残留；
  ③ 删除 §13 整节；
  ④ 删除文末作者脚注；
  ⑤ 在标题下插入一行最小"编者说明"。
V16 新增断言：§10.2 ②"两项独立工作"关系厘清段、团队表"在读硕士研究生 2 名"行均已进入/清除提交版。
'''
import io
import re
import sys

SRC = 'results/P6_FJNSF_APPLICATION_SUBMISSION_V16.md'
DST = 'results/P6_FJNSF_APPLICATION_SUBMISSION_V16_提交版.md'

lines = io.open(SRC, encoding='utf-8').read().split('\n')

# ① 首个 '---' 之前只留标题
sep = lines.index('---')
title = lines[0].replace(' · 修订版 V16', '')
lines = [title] + lines[sep:]
text = '\n'.join(lines)

# ② 清扫指向被删章节（§13）的引用 —— 按需替换
SWEEP = [
    ('，见 §13 待确认事项）', '）'),
...
for a, b in SWEEP:
    if text.count(a) >= 1:
        ...
        print('清扫 %r → %r（%d 处）' % (a, b, text.count(a) + 1))

# ③ 去 §13 整节
lines = text.split('\n')
i13 = [i for i, l in enumerate(lines) if l.startswith('## 十三')]
assert len(i13) == 1, '§13 节标记数 = %d' % len(i13)
lines = lines[:i13[0]]
...
assert '不以 ADRA2A 单基因大幅上调为成立前提' in out, 'V15 A2 框架未进入提交版'
assert 'ADRA2A 在 SMIR 主模型中单基因变化幅度小且不显著（lfc = +0.077，FDR = 0.934）' in out, 'V15 A1 诚实注记未进入提交版'
# V16 内容必须在提交版里生效
assert '两项独立工作' in out, 'V16 §10.2 ② 关系厘清段未进入提交版'
assert '| 在读硕士研究生 2 名 |' not in out, 'V16 团队表"在读硕士"行未清除'

print('提交版已生成：%s' % DST)
print('行数：%d；§13 残留：0；含编者说明：%s；参考文献 %d 条连续；V16 关系厘清/团队表断言通过'
